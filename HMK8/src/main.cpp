...
int main(int argc, char* argv[]){
	...
	for(int i = 0; i < argc; i++){
		...
		if(s == "-f")
			...
			memory_fault = std::stof(argv[i+1]);
		else if(s == "-g")
	...
	for(int i = 0; i < ios+1; i++){
		...
		while(true){//FIFO Scheduler with modifications for the different schedulers
			...
			if(cpus > 0 && available_devices[readyqueue.peek().get_device()] && readyqueue.peek().get_memory() < page_size){
				cpus--;
				if(readyqueue.peek().get_device() > 0 )
					available_devices[readyqueue.peek().get_device()] = false;
				page_size -= readyqueue.peek().get_memory();
